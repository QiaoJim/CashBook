...
public class LineChartView2 extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < YLabel.length; i++) {
            YLabel[i] = mPreferences.getInt("month_y_scale", 50) * i + "";
        }

        for (int i=0;i<time_data.size();i++) {
            Log.v("---- on draw 2---", "ok     " + time_data.get(i) + "    " + money_data.get(i));
        }
        ...
        //Y轴箭头
        canvas.drawLine(XPoint+25, YPoint - YLength, XPoint +18, YPoint - YLength + 10, paint_line);
        canvas.drawLine(XPoint+25, YPoint - YLength, XPoint +32, YPoint - YLength + 10, paint_line);
        ...
        canvas.drawLine(screen_width - 70 , YPoint - YScale *2 + 6,screen_width - 60, YPoint - YScale *2, paint_line);
        //画x轴提示
        canvas.drawText("月份",screen_width - 130,YPoint - YScale, paint_text);

        if (time_data.size()>0) {
            //画 时间
            canvas.drawText(time_data.get(0) + "", XPoint + 0.6f*XScale, YPoint - YScale, paint_text);
            //画虚线
            Path path = new Path();
            path.moveTo(XPoint + XScale, YPoint - YScale * 2);
            path.lineTo(XPoint + XScale, YPoint - money_data.get(0) * YScale - YScale * 2);
            canvas.drawPath(path, paint_xuxian);
            //画圆点
            canvas.drawCircle(XPoint + XScale, YPoint - money_data.get(0) * YScale - YScale * 2,
                    6f,paint_line);

            if (money_data.size() > 1) {
                for (int i = 1; i < money_data.size(); i++) {
                    //画折线
                    canvas.drawLine(XPoint + i * XScale, YPoint - money_data.get(i - 1) * YScale - YScale * 2,
                            XPoint + (i + 1) * XScale, YPoint - money_data.get(i) * YScale - YScale * 2, paint_line);
                    //画圆点
                    canvas.drawCircle(XPoint + (i + 1) * XScale, YPoint - money_data.get(i) * YScale - YScale * 2,
                            6f,paint_line);

                    //坐标底部时间
                    canvas.drawText(time_data.get(i) + "", XPoint + (i+0.6f) * XScale, YPoint - YScale, paint_text);
                    //连接虚线
                    Path path1 = new Path();
                    path1.moveTo(XPoint + (i + 1) * XScale, YPoint - YScale * 2);
                    path1.lineTo(XPoint + (i + 1) * XScale, YPoint - money_data.get(i) * YScale - YScale * 2);
    ...
    private void initLength() {
        ...
        XScale = (int) (20 * this.getResources().getDisplayMetrics().density + 0.5f);
